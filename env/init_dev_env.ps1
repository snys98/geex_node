...
Write-Output "3. a dns server will be add into you docker network, so that *.dev.geex.io can target to your local machine."
Write-Output "4. a dev cert will be installed into you local machine trusted root store, so that you can access https://*.dev.geex.io for local development, you'll need to restart your browser to ensure the cert is loaded."
...
$cert = Get-CCertificate -FriendlyName "dev.geex.io" -StoreLocation LocalMachine -StoreName Root
$pwd = ConvertTo-SecureString -String "dev.geex.io" -AsPlainText -Force
if ($null -eq $cert) {
  $cert = New-SelfSignedCertificate -CertStoreLocation Cert:\LocalMachine\My -Subject dev.geex.io -DnsName dev.geex.io, *.api.dev.geex.io, *.dev.geex.io -FriendlyName "dev.geex.io" -NotAfter (Get-Date).AddYears(1000)
  mkdir ./certs -ErrorAction Ignore
  Export-PfxCertificate -Cert "Cert:\LocalMachine\My\$($cert.Thumbprint)" -FilePath "./certs/dev.geex.io.pfx" -Password $pwd
  openssl pkcs12 -in "./certs/dev.geex.io.pfx" -nodes -out ./certs/dev.geex.io.pem -passin pass:dev.geex.io
  openssl pkcs12 -in "./certs/dev.geex.io.pfx" -out ./certs/dev.geex.io.crt -clcerts -nokeys -passin pass:dev.geex.io
  openssl pkcs12 -in "./certs/dev.geex.io.pfx" -out ./certs/dev.geex.io.key -nocerts -nodes -passin pass:dev.geex.io
  Install-CCertificate -Path "./certs/dev.geex.io.pfx" -StoreLocation LocalMachine -StoreName Root -Password $pwd
}
